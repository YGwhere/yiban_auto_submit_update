...
class YiBan:
    ...
    def submit(self, data, extend, person_name):
        data = data.replace("刘鑫", person_name)
        data = data.replace("2020-11-12 12:13", time.strftime("%Y-%m-%d %H:%M"))
        params = {
            "data": data,
            "extend": extend
        }
    ...
    def getDetailTaskForm(self):
        return self.request("https://api.uyiban.com/workFlow/c/my/form/%s?CSRF=%s" % (self.WFId, self.CSRF),
                            cookies=self.COOKIES)
    ...
    for index, account_detail in enumerate(allAccount):
        try:
            ...
            if yb.login() is None:
                ...
                continue
            result_auth = yb.auth()
            person_name = result_auth["data"].get("PersonName")
            ...
            for i in all_task["data"]:
                ...
                task_Form = yb.getDetailTaskForm()
                ...
                submit_result = yb.submit(allData[0], json.dumps(ex, ensure_ascii=False), person_name)
